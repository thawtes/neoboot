...
#zwraca struture folderu zip - vuplus/vumodel
def getImageFolder():
    if os.path.isfile('/proc/stb/info/vumodel'):
        BOX_NAME = getBoxVuModel()
...
def getImageInNeoBoot():
    try:
        return open('/proc/stb/info/vumodel').readlines().strip()
    except:
        pass
...
def getTypBox():
    if not fileExists('/etc/typbox'):
        os.system('touch /etc/typbox')
        f2 = open('/etc/hostname', 'r')
        mypath2 = f2.readline().strip()
        f2.close()
        if mypath2 == 'vuuno4k':
            out = open('/etc/typbox ', 'w')
            out.write('uno4k ')
            out.close()
        elif mypath2 == 'vuultimo4k':
            out = open('/etc/typbox ', 'w')
            ...
            out.close()
    try:
        lines = open('/etc/typbox', 'r').readlines()
        typbox = lines[0][:-1]
    except:
        typbox = 'not detected'
